...
def safe_parse(raw_json):
    # your code here
    try:
        json_object = ujson.loads(raw_json.encode('utf-8'))
        pair= tuple((json_object['user']['id_str'],json_object['text']))
    except (ValueError, KeyError):
        return None
    return pair

# your code here
RDD = rdd.map(safe_parse)
#Removing the None values##
RDD=RDD.filter(lambda x:(x!=None)).cache()
...
# your code here
count=RDD.map(lambda (a,b):a).distinct().count()
...
# your code here
import pickle
pkl=open('../Data/users-partition.pickle', 'rb')
partition=pickle.load(pkl)
partitionBC = sc.broadcast(partition)
...
RDD_group_count = RDD.map(lambda (a,b):partitionBC.value[a] if partitionBC.value.has_key(a) else 7).countByValue()
...
# your code here
print_post_count(RDD_group_count.items())
...
# your code here
counts=RDD.flatMap(lambda (a,b):tok.tokenize(b.encode('utf-8'))).distinct()
print_count(counts)
...
# your code here
counts2=RDD.flatMap(lambda (a,b):[(token,a) for token in set(tok.tokenize(b.encode('utf-8')))]).distinct().cache()
counts_filtered=counts2.groupByKey().mapValues(lambda x:len(set([a for a in x]))).filter(lambda (a,b):b>=100).sortBy(lambda a: -a[1]).cache()

print_count(counts_filtered)
print_tokens(counts_filtered.take(20))
...
counts3=counts2.map(lambda (a,b):(a,partitionBC.value[b]) if (partitionBC.value.has_key(b)) else (a,7)).cache()
RDD_group_token=counts3.subtractByKey(counts3.subtractByKey(counts_filtered)).cache()


# In[344]:

token_count=counts_filtered.collectAsMap()


# In[345]:

for grp in range (8):
    RDD_group_token_pop=RDD_group_token.filter(lambda (a,b):b==grp).groupByKey().mapValues(lambda x:len([a for a in x]))    .map(lambda (a,b):(a,get_rel_popularity(b,token_count[a])))    .sortBy(lambda a: a[0]).sortBy(lambda a: -a[1])
    print_tokens(RDD_group_token_pop.take(10),grp)
